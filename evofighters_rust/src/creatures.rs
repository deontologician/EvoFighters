...
impl Creature {
    pub(in creatures) fn new(id: usize,
           dna: dna::DNA,
           generation: usize,
    ...
    pub fn carryout(&mut self,
                    ...
                    rng: &mut RngState) -> arena::FightStatus {
        if self.is_feeder() {
...
impl Creatures {
    ...
    pub fn random_creature_or_feeder(&mut self) -> Creature {
        let index = self.rng.rand_range(
            0, self.creatures.len() + self.feeder_count);
    ...
    pub fn mate(&mut self, p1: &mut Creature, p2: &mut Creature)
                -> (Option<Creature>, GlobalStatistics) {
        let (child_dna, stats) = dna::DNA::combine(
            &mut p1.dna, &mut p2.dna, &mut self.rng);
